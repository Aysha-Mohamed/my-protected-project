  ...
  for (const element of content) {
    ...
    if (state === "body") {
      if (namedStyle === "HEADING_1") {
        ...
        const itemText = rawText.startsWith("●  \t")
          ? rawText.substring("●  \t".length).trim()
          : rawText;
        if (
          ...
          currentSection.content[currentSection.content.length - 1].data.push(
            itemText
          );
        } else {
        ...
        if (links && links.length === 1 && text === `{${links[0].id}}`) {
          currentSection.content.push({
            type: "image",
            data: links[0].url,
          });
